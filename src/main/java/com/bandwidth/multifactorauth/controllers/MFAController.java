...
import com.bandwidth.multifactorauth.exceptions.ErrorWithRequestErrorException;
import com.bandwidth.multifactorauth.exceptions.ForbiddenRequestErrorException;
import com.bandwidth.multifactorauth.exceptions.UnauthorizedRequestErrorException;
...
public final class MFAController extends BaseController {
    ...
    private ApiResponse<TwoFactorVoiceResponse> handleCreateVoiceTwoFactorResponse(
        ...
        if (responseCode == 400) {
            throw new ErrorWithRequestErrorException("If there is any issue with values passed in by the user", context);
        }
        if (responseCode == 401) {
            throw new UnauthorizedRequestErrorException("Authentication is either incorrect or not present", context);
        }
        if (responseCode == 403) {
            throw new ForbiddenRequestErrorException("The user is not authorized to access this resource", context);
        }
        if (responseCode == 500) {
            throw new ErrorWithRequestErrorException("An internal server error occurred", context);
        }
    ...
    private ApiResponse<TwoFactorVerifyCodeResponse> handleCreateVerifyTwoFactorResponse(
        ...
        if (responseCode == 429) {
            throw new ErrorWithRequestErrorException("The user has made too many bad requests and is temporarily locked out", context);
        }
        if (responseCode == 500) {
            throw new ErrorWithRequestErrorException("An internal server error occurred", context);
        }
